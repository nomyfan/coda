...
fn model_output_overrides_inherit_each_unspecified_field() {
    let text = BASE.replace(
        ...
        "context_window = 64000, output_limits = {single_bytes = 8192}",
    );
    let text =
...
fn invalid_resource_tables_and_removed_fields_fail_loading() {
    for suffix in [
        ...
        "[resources]\noutput = 'invalid'",
        "[resources.output]\ncapture_memory_bytes = 4194304\n[resources.ptc]\nhost_buffer_bytes = 4194304",
        "[resources.output.model]\nsingle_bytes = 65536\nbatch_bytes = 8192",
    ] {
        assert!(
...
fn global_path_budget_is_checked_after_resolving_relative_root() {
    ...
    let error = parse_server_config(&text, &base_dir)
        .unwrap_err()
        .to_string();
    assert!(error.contains("resources.output.model.single_bytes"));

    let minimum = ModelOutputLimits::minimum_response_bytes(&base_dir.join("output")).unwrap();
    let text = text.replace("single_bytes = 1024", &format!("single_bytes = {minimum}"));
    let config = parse_server_config(&text, &base_dir).unwrap();
    assert_eq!(config.resources.output.model.single_bytes, minimum);
    assert_eq!(
        config.providers[0].models[0].output_limits.single_bytes,
        minimum
    );
}
...
fn every_model_override_must_fit_the_resolved_output_paths() {
    ...
    let text = BASE.replace(
        "models = [{ id = \"model\", context_window = 64000 }]",
        "models = [{ id = \"inherited\", context_window = 64000 }, { id = \"small\", context_window = 64000, output_limits = {single_bytes = 1024} }]",
    );
    let text = format!("{text}\n[resources.output]\nroot = 'output'\n");
    let error = parse_server_config(&text, &base_dir)
        .unwrap_err()
        .to_string();
    assert!(error.contains("provider 'test' model 'small' output_limits.single_bytes"));
    ...
    assert_eq!(
        config.providers[0].models[1].output_limits.single_bytes,
        minimum
    );
    assert_eq!(
        config.providers[0].models[1].output_limits.batch_bytes,
        config.resources.output.model.batch_bytes
    );
}
