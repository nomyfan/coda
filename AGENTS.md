...
- **`coda_tools`** — Built-in tool implementations and the tool spec system. Provides 10 declarable built-in tools (`shell`, `read_file`, `write_file`, `edit_file`, `ls`, `grep`, `glob`, `read_todos`, `write_todos`, `run_javascript`) plus the two background follow-up tools `task_output` and `task_kill`, which are injected rather than declared (`background_specs`, see Agent Configuration). Also `TodoItem`, the `ToolSpec` factory trait (with `name()` metadata), `BuildContext`, `PrebuiltToolSpec`, and name-based resolution (`builtin_specs()`, `spec_by_name`). Depends on `coda_core` and on `coda_execution` — for the shared process-group primitive every shelling-out tool uses, and for the task registry `shell`, `task_output` and `task_kill` drive — and uses `coda_ptc` only for the runner spec.
...
`task_output` and `task_kill` are the exception to all of the above: they are not declarable and not granted per agent. Every agent gets both exactly when the session has a background registry, and none of them do when it has none — the same condition that decides whether `shell` exposes `run_in_background` at all, so the three appear and disappear together. Naming either one in `tools` is an unknown-tool error. (When a `capabilities` declaration lands, background becomes one of those; until then it is simply on whenever the archive root opens.)


### Background Subagents

Only the actual session root thread may pass `run_in_background: true` to an available `agent__*` tool. The default remains synchronous. The runtime checks this independently of the tool schema. Drivers and mutable process state are per logical process; stateful calls remain isolated by parent thread and reject overlapping foreground/background invocations immediately. Completed foreground subagent drivers retire before their Reply releases the call ledger; later stateful invocations restore their checkpoint. Completed JoinSet entries are reaped as messages are delivered.
