...
Only the actual session root thread may pass `run_in_background: true` to an available `agent__*` tool. The default remains synchronous. The runtime checks this independently of the tool schema. Drivers and mutable process state are per logical process (the protocol still calls its identity `thread_id`); stateful calls remain isolated by parent thread and reject overlapping foreground/background invocations immediately. Completed foreground subagent drivers retire before their Reply releases the call ledger; later stateful invocations restore their checkpoint. Completed JoinSet entries are reaped as messages are delivered.
