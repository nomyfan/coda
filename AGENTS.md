...
- **`coda_output`** — Process-wide bounded output storage: fd-relative safe directories and root locking, streaming head/tail capture, sequential channel files, per-result/session/service quotas, durable sealing, retention/eviction and UTF-8-safe previews. `coda_execution`, foreground tools and PTC reuse it. `coda_core::output` owns shared data, receipt and budget contracts. Model rendering preserves complete paths and separates storage failure from execution status. See `docs/implementation/tool-output-limits.md`.
