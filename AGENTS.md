...
- **Database** — required `[database]` table with a `url` (supports `${VAR}` expansion). PostgreSQL 13 or newer (the storage tests call the built-in `gen_random_uuid()`, which older releases only expose through `pgcrypto`). Sessions live in PostgreSQL: `sessions` is the aggregate root, with `thread_checkpoints`, `messages` (one row per message) and `runtime_snapshots` hanging off it via a composite foreign key with `ON DELETE CASCADE`. Migrations are embedded in the binary and run at startup, so deploying creates or updates the schema. The storage tests need a live database and are gated behind the `pg-tests` feature (`DATABASE_URL` + `cargo test -p coda_server --features pg-tests --test storage_pg`); point it at a throwaway database, never a real one.
