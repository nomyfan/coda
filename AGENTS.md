...
- **`ToolSpec` / `BuildContext`** (`coda_tools::spec`) — `ToolSpec` is a factory trait for creating tool instances; `BuildContext` carries the workspace directory, the todo store, and the file lock registry during tool construction.
