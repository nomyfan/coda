...
- **`coda_process`** — Everything that runs a child process. `process::GroupedChild` is the primitive both tenses share: a command pinned inside a fresh, sentinel-led process group that can be SIGKILLed whole, used by `coda_tools`' foreground path (`shell`, `grep`, `glob`, `ls`) as much as by background work. On top of it, `BackgroundProcesses` is the session-scoped registry for tasks that outlive a turn — spawn/observe/kill, completion notices, and a per-session on-disk output spool (bounded ring files under a locked archive root). Depends only on `coda_core` among Coda crates.
