...
Existing sessions keep their persisted model binding. If its provider/model is no longer configured, or its saved reasoning effort is unsupported, the hub opens a `ReadOnly` view directly from storage without constructing a runtime or recovering background tasks. Snapshots and catalog entries carry `access`; read-only execution commands fail with `SESSION_READ_ONLY` (`-32007`). History, pending approvals and retained task results remain readable; rename and delete remain available. Restoring the configuration takes a server restart and a fresh attach. Shell allow rules can only be saved through an accepted `resume.allow_patterns`; there is no standalone `add_allow_pattern` RPC.
