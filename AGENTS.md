...
`task_output` and `task_kill` are the exception to all of the above: they are not declarable and not granted per agent. Every agent gets both exactly when the session's background registry has storage behind it, and none of them do when it has not — the same condition that decides whether `shell` exposes `run_in_background` at all, so the three appear and disappear together. Naming either one in `tools` is an unknown-tool error. (When a `capabilities` declaration lands, background becomes one of those; until then it is simply on whenever the archive root opens.)
