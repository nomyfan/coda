...
Sub-agents are declared one-per-directory under `.coda/agents/<name>/AGENT.md`: YAML frontmatter (`description`, `mode` = stateful/stateless, `tools`, `subagents`) plus a markdown body used as the system prompt. They become sub-agents of the top-level `coda` agent and may reference one another by name to form deeper graphs (sharing allowed).
...
Tools resolve by name against built-ins plus prebuilt tools (e.g. MCP tools from `mcp.json`). Unknown tools, duplicate agent names, dangling sub-agent references, and tool/sub-agent namespace conflicts are hard startup errors. Sub-agents unreachable from `coda` are ignored with a warning.
