...
crates/
  ...
  ├── coda_output  — shared archive path safety and bounded output primitives
...
- **`coda_execution`** — Child-process execution and the shared background task registry. `process::GroupedChild` is the primitive both tenses share: a command pinned inside a fresh, sentinel-led process group that can be SIGKILLed whole, used by `coda_tools`' foreground path (`shell`, `grep`, `glob`, `ls`) as much as by background work. On top of it, `BackgroundTasks` is the session-scoped registry for tasks that outlive a turn — spawn/observe/kill, completion notices, and a per-session on-disk output spool (shell ring files and immutable subagent results under a locked archive root). `TaskKind` distinguishes shell and subagent tasks; the registry owns result quotas, pending delivery markers, and owned shell cancellation. Depends on `coda_core` and `coda_output` among Coda crates.
...
- **`coda_output`** — Shared fd-relative archive directory operations and root locking, extracted from `coda_execution`, plus bounded head/tail previews. `coda_execution` now reuses this crate's directory implementation. The remaining output store and runtime integration are tracked in `docs/implementation/tool-output-limits.md`; the background ring storage is still in use until that migration completes.
