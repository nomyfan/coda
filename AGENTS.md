...
- **Heartbeat** — optional `[heartbeat]` table with `interval_secs` (default 30), how often an idle WebSocket connection emits a protocol-level Ping so proxies and load balancers don't cut it. The silence budget after which a peer is declared gone is derived as three intervals rather than being its own knob, so no configuration can produce a timeout firing before the first Pong could arrive. Browsers answer Pings automatically, so this needs nothing from the client — and the client cannot send Pings itself, since the browser WebSocket API exposes no such call.
