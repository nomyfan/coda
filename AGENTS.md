...
Each generated assistant message stores optional `generation` metadata (`provider_id`, the requested API `model_id`, and `reasoning_effort`), including partial aborted replies. Old and synthetic messages may omit it. This records the actual process profile, including subagent overrides; it survives fork/rewind with the message and is never sent to the provider. Historical model labels use this metadata even if the model has since been removed.
