...
Background approvals are independent of root running state and keyed by `(pid, parent_message_id, call_id)`. `resume` is an acknowledged JSON-RPC request; optional shell allow patterns are saved only after an effective approval is accepted. Pending approvals block new root input and automatic notice turns. Background completion notices save the root opening and a `task_notice_receipts` row atomically, then wake the root and acknowledge the archive. A successful root `task_output` that reads the complete terminal result also records a receipt in the same checkpoint transaction; queued notices for that task then need no new turn. Running or partial reads, panel reads, and subagent reads do not acknowledge delivery to root. Receipts survive rewind and are not copied by fork. Relay gaps refresh live state without stopping execution. SetModel, manual compact, fork and rewind require no live background subagent or outstanding scope cleanup.
