...
After modifying Rust code, always run `cargo clippy` and `cargo test` as a final check, plus `cargo check -p coda_server --features pg-tests --all-targets` — neither of the first two builds the feature-gated storage tests (see Database below).
...
crates/
  ...
  ├── coda_execution — child-process execution: killable process groups + background tasks
  ├── coda_output  — bounded capture, retained output files, quotas and model previews
...
- **`coda_agent`** — Agent runtime. `AgentSpec` is declarative source, `AgentTeam` validates the graph and builds session-bound `Program`s (prompt, tools and callable targets). Each `Process` references a program and owns a `ProcessId`, independent memory and execution state. `ProcessRuntime` routes by process identity and creates fixed-process drivers on demand; drivers may retire while checkpoints retain the process. `Session` wraps input, approval, events and shutdown. `ProcessGroupId` identifies execution groups: foreground by turn, background by task. `RunConfig` and `ModelProfile` retain session model overrides and approval configuration.
- **`coda_tools`** — Built-in tool implementations and the tool spec system. Provides 9 declarable built-in tools (`shell`, `read_file`, `write_file`, `edit_file`, `ls`, `grep`, `glob`, `read_todos`, `write_todos`). The runtime injects `run_javascript` for the `ptc` capability and `task_output` / `task_kill` for `background`; `list_javascript_tools` is synthesized alongside the runner (see Agent Configuration). Also `TodoItem`, the `ToolSpec` factory trait (with `name()` metadata), `BuildContext`, `PrebuiltToolSpec`, and name-based resolution (`builtin_specs()`, `spec_by_name`). Depends on `coda_core` and on `coda_execution` — for the shared process-group primitive every shelling-out tool uses, and for the task registry `shell`, `task_output` and `task_kill` drive — and uses `coda_ptc` only for the runner spec.
- **`coda_execution`** — Child-process execution and the shared background task registry. `process::GroupedChild` is the primitive both tenses share: a command pinned inside a fresh, sentinel-led process group that can be SIGKILLed whole, used by `coda_tools`' foreground path (`shell`, `grep`, `glob`, `ls`) as much as by background work. On top of it, `BackgroundTasks` is the session-scoped registry for tasks that outlive a turn — spawn/observe/kill, completion notices, and durable task lifecycle metadata. `TaskKind` distinguishes shell and subagent tasks; the registry owns pending delivery markers and shell cancellation, while all payload capture, retention and quotas belong to `coda_output`. Depends on `coda_core` and `coda_output` among Coda crates.
- **`coda_ptc`** — Provider-independent programmatic tool calling runtime. Runs bounded ES2020 JavaScript in a short-lived rquickjs runtime on a dedicated thread, exposes only a persisted generation-time subset of built-in tools through an async host bridge, and owns console/result limits, watchdogs, and JS execution reports. Depends on `coda_core` and `coda_output` among Coda crates.
- **`coda_output`** — Process-wide bounded output storage: fd-relative safe directories and root locking, streaming head/tail capture, sequential channel files, per-result/session/service quotas, durable sealing and retention/eviction. `coda_execution`, foreground tools and PTC reuse it. `coda_core::output` owns shared data, receipt and budget contracts, and the line-aware previews whose notes give exact line numbers and byte offsets into the saved files. Model rendering preserves complete paths and separates storage failure from execution status. See `docs/implementation/tool-output-limits.md`.
...
- **Database** — required `[database]` table with a `url` (supports `${VAR}` expansion). PostgreSQL 13 or newer (for the built-in `gen_random_uuid()`). Sessions live in PostgreSQL: `sessions` is the aggregate root, with `process_checkpoints`, `messages` (one row per message) and `runtime_snapshots` hanging off it via composite foreign keys with `ON DELETE CASCADE`. Tool state is a `state` column on `messages`, holding the `{kind: value}` map the call recorded in that message; a fork copies it with the row and a rewind deletes it with the row, so neither needs SQL of its own. Migrations are embedded in the binary and run at startup, so deploying creates or updates the schema. Connections are **not** TLS, so the database must be on a trusted network. The storage and database-backed RPC tests need a live database and are gated behind the `pg-tests` feature (`DATABASE_URL` + `cargo test -p coda_server --features pg-tests`); point it at a throwaway database, never a real one. For local development, run the full feature-enabled suite with `DATABASE_URL=postgres://postgres:postgres@localhost:35432/coda_test cargo test --features pg-tests`. Because plain `cargo test` never builds that target, a signature change can break it unnoticed — `cargo check -p coda_server --features pg-tests --all-targets` catches that without a database.

  Queries go through diesel's DSL, type-checked against `app/coda_server/src/schema.rs` — **a generated file; `diesel migration run` rewrites it, never hand-edit**. Typed `jsonb` columns go through `Json<T>` (`src/jsonb.rs`), since diesel only ships `Jsonb` support for `serde_json::Value`. Foreign keys are composite — session-owned rows use `(workspace_id, session_id)` — and diesel's `joinable!`/`belongs_to` cannot express them, so joins spell out `.on(...)`.
...
- **Resource limits** — `[resources.output]` configures `root`, `capture_memory_bytes`, `result_max_bytes`, `session_disk_bytes`, `total_disk_bytes` and `retention_secs`; `[resources.output.model]` sets `single_bytes` (16 KiB default) and `batch_bytes` (64 KiB). Each provider model may override either field in `output_limits`. `[resources.ptc]` configures execution timeout, JS heap, native buffers, host call count/concurrency and final report size. PTC intermediates are not truncated or limited by cumulative traffic. Settings are startup-only and validated together, including the minimum space for complete output paths. See `examples/coda-server.toml` and `docs/tool-output-storage.md` for defaults and deployment.
...
Existing sessions keep their persisted provider/model/effort binding and optional family. Opening a session with no saved family enrolls it from its original model's current nonempty family; once saved, family is fixed. The user may manually switch an idle session to another model in that family, including across providers or after a Preview model is removed. With no family, only the original model's effort can change. A missing original model, unsupported effort, or changed/removed configured family opens a read-only view without constructing a runtime. All selection requests, including effort changes and retries, must satisfy the fixed family; the server also checks images in every inherited process's effective history, excluding explicit subagent model overrides.

`set_model` persists the full binding before rebuilding the runtime and returns a full snapshot with `model_family`, `model_candidates`, and `runtime_open_error`. A failed rebuild retains the committed selection in read-only state for manual retry. An uncertain commit disables execution until a locking read confirms the original transaction's outcome; ordinary reads cannot establish rollback. Snapshots and catalog entries carry `access`; read-only execution commands fail with `SESSION_READ_ONLY` (`-32007`), while manual model recovery, history, pending approvals, retained task result reads, rename, and delete remain available. Configuration changes require a server restart. Shell allow rules can only be saved through an accepted `resume.allow_patterns`; there is no standalone `add_allow_pattern` RPC.

Each generated assistant message stores optional `generation` metadata (`provider_id`, the requested API `model_id`, `reasoning_effort`, and optional `reported_model_id`), including partial aborted replies. The runtime captures the process's request profile, including subagent overrides, and the last valid model reported by the upstream stream. The reported model may be an alias and does not verify a backend version or serving provider; missing reports stay unknown. Old and synthetic messages may omit metadata. It survives fork/rewind with the message and is never sent to the provider. Historical model labels prefer the reported model and mark a fallback to the requested model explicitly, even if the model has since been removed.
...
| mode | auto-approves |
| --- | --- |
| `explore` | `ls`, `read_file`, `glob`, `grep`, `read_todos`, `write_todos`, `run_javascript`, `task_output` |
| `accept_edits` (default) | the above plus `write_file`, `edit_file` |
| `yolo` | everything, `shell` and `task_kill` included |
...
- **`AgentSpec` / `AgentTeam`** (`coda_agent::spec`) — `AgentSpec` is plain per-agent data, including capabilities and sub-agents referenced by name. `AgentTeam::new(root, subagents)` validates the whole set once (unique names, resolvable references, tool/sub-agent namespace conflicts and runtime-reserved names for both tools and sub-agents; sub-agents unreachable from the root are dropped; each retained sub-agent's `agent__`-prefixed tool name must fit the 64-character provider limit) so holding one proves it sound; `AgentTeam::build(workspace_dir)` then constructs session-bound `Arc<Program>` definitions without process memory (infallibly).
- **`Session`** (`coda_agent::session`) — High-level API for callers: send tasks, consume events, resume from suspension, and shut down sessions. `SessionBuilder::team(&AgentTeam, workspace_dir)` borrows the team and builds the programs at `open()`.

### Agent Configuration (file-based)

Sub-agents are declared one-per-directory under `.coda/agents/<name>/AGENT.md`: YAML frontmatter (`description`, `mode` = stateful/stateless, `tools`, `capabilities`, `subagents`, `workspace`, `model`, `reasoning_effort`) plus a markdown body used as the system prompt. They become sub-agents of the top-level `coda` agent and may reference one another by name to form deeper graphs (sharing allowed).
...
The `workspace:` frontmatter roots an agent at its own directory (its tool root and knowledge source) — absolute, or relative to the root workspace; absent inherits the root (session) workspace. A `workspace:` that doesn't resolve to an existing directory is a hard startup error. Agents sharing a workspace share that workspace's knowledge handles (skills + custom instructions) + watcher. This is a default cwd/relative-root, **not** a sandbox — tools can still reach outside it. A per-agent workspace does **not** load its own `.coda/agents` (agent topology is defined only in the root workspace). Only the **workspace knowledge** (`AGENTS.md` + skills) hot-reloads, via the watcher; agent bodies and all frontmatter (`tools`/`capabilities`/`subagents`/`mode`/`model`/`workspace`) are read once at load and need a restart to change. MCP tools and the approval policy remain rooted at the session workspace / session-global.

The `coda` agent itself is configured by an optional `.coda/agents/AGENT.md` (a bare file, not a directory): its `tools`, `capabilities`, `subagents`, and body each *explicitly override* a default when present (otherwise: all ordinary tools, all capabilities, the auto-attached unreferenced agents, and the built-in `system-prompt.md` base prompt). `coda` is always present.

Tools resolve by name against built-ins plus prebuilt tools (e.g. MCP tools from `mcp.json`). A name ending in `*` is a prefix pattern — `mcp__example__*` enables every tool that server exposes; a bare `*` is not a wildcard. Every agent defaults to all registered ordinary tools, including `ask_user` and MCP tools. A list or `include` selects a replacement set; `exclude` subtracts from it. With only `exclude`, the starting set is all ordinary tools. `tools: []` or `include: []` gives no ordinary tools; capabilities and sub-agent targets remain independently configured. Unknown plain tool names, duplicate agent names, dangling sub-agent references, and tool/sub-agent namespace conflicts are hard startup errors; a pattern that matches nothing only warns. Sub-agents unreachable from `coda` are ignored with a warning.

Every agent also defaults to all supported capabilities, currently `background` and `ptc`. An explicit list replaces this default: `capabilities: [ptc]` enables only PTC, and `capabilities: []` disables both. Names are lowercase; unknown names, invalid types, and explicit null are errors; duplicates are deduplicated. Each agent uses its own tools and capabilities regardless of which agent calls it. A shared child has the same configuration under every caller.

- **`background`** — With a session background registry, enables `task_output`, `task_kill`, and the `shell` background option if the agent has `shell`. Without either the capability or the registry, those entries are hidden and a raw background shell request fails before starting a process. Disabling an agent's capability leaves the session registry, cleanup, result panel, and root completion notices intact.
- **`ptc`** — Enables `run_javascript` and `list_javascript_tools` when the agent has eligible host tools allowed by the current approval policy. Both entries are hidden when that set is empty. Calls are restricted by their generation-time snapshot and the current tools and approval policy; restored snapshots cannot re-enable a disabled capability.

The four capability tool names are reserved: they cannot appear as exact entries in ordinary `tools` include/exclude lists, as registered `ToolSpec` names, or as sub-agent names, even when the corresponding capability is disabled. Use `capabilities` to configure them. Prefix patterns only match the ordinary registry, so `run_*` does not select `run_javascript`. `AgentTeam::new` enforces reserved names for Rust callers too, checking sub-agent definitions and references before unreachable nodes are dropped. Native Rust `AgentSpec.tools` remains an explicitly resolved list; its empty vector means no ordinary tools.


### Background Subagents

Only the actual session root thread, with its own `background` capability enabled and a session background registry, may pass `run_in_background: true` to an available `agent__*` tool. The target may have `background` disabled; the capability controls what it can start itself, while execution-group cancellation and cleanup still apply. The default remains synchronous. The runtime checks this independently of the tool schema. Drivers and mutable process state are per logical process; stateful calls remain isolated by parent thread and reject overlapping foreground/background invocations immediately. Completed foreground subagent drivers retire before their Reply releases the call ledger; later stateful invocations restore their checkpoint. Completed JoinSet entries are reaped as messages are delivered.

A background subagent owns one `ProcessGroupId`, inherited by all synchronous descendants and their tools. Root turn cancellation leaves that scope running. Stopping its task cancels the scope and its owned background shell processes; a normally completed subagent can still own running shell tasks. `task_output` pages through immutable subagent answers using explicit byte offsets; shell output is incremental per consumer, with offsets committed alongside tool results. The web task panel reads agent answers through `get_task_result` and uses `subtree_active` to keep the parent stop action available.

Any checkpoint failure inside a background scope closes that scope, revokes its approvals, terminates its drivers without relying on Reply, and quarantines its member threads. The storage `abort_scope` transaction cleans pending execution and records invocation fences in `aborted_executions`; only successful persistent cleanup permits thread reuse. Session cold-open performs the same cleanup before bootstrap and marks unfinished archived tasks Interrupted; it never resumes background execution.

Background approvals are independent of root running state and keyed by `(pid, parent_message_id, call_id)`. `resume` is an acknowledged JSON-RPC request; optional shell allow patterns are saved only after an effective approval is accepted. Pending approvals block new root input and automatic notice turns. Background completion notices save the root opening and a `task_notice_receipts` row atomically, then wake the root and acknowledge the archive. A successful root `task_output` that reads the complete terminal result also records a receipt in the same checkpoint transaction; queued notices for that task then need no new turn. A subagent also acknowledges a complete terminal read of its own shell task; its checkpoint stores the receipt atomically. Running or partial reads, panel reads, and subagent reads of other tasks do not acknowledge delivery to root. Unread shell completions still notify root even after their parent agent finishes. Receipts survive rewind and are not copied by fork. Relay gaps refresh live state without stopping execution. SetModel, manual compact, fork and rewind require no live background subagent or outstanding scope cleanup.

Process groups contain executions identified by `(pid, invocation_id)`, not permanently assigned processes. Old group cleanup must not affect a later invocation on the same pid. The driver preflights the entire pending batch before starting any calls, rejecting every duplicate stateful target; the runtime handles individual admission. During exit, new work is refused but in-flight replies are captured in the runtime snapshot until shutdown completes.
