...
- [x] **[risk validation] `open_session` request + `snapshot` notification, end to end.**
      ...
      Verification: open a session, reconnect mid-turn, and take over from a second tab — transcript, approvals, and running state match `main`. *(Landed as `attach_core`; `open_session`→`snapshot` result verified against a live server — full transcript rendered from the request result.)*

- [x] **[core envelope] Freeze the envelope + error codes and the dispatcher shape.**
      ...
      Verification: `cargo test` round-trips for envelope decode/encode and each error code; a non-JSON frame yields `-32700` with id `null`, a structurally-invalid request `-32600`, and an unknown-method *request* `-32601`; and — asserting the notification rule — an unknown-method *notification* and an invalid-params *notification* each yield **no** `RpcOutgoing` (log-and-drop), while the same payloads as requests yield `-32601`/`-32602`. *(Landed: `rpc.rs` with unit tests; the dispatch split is `handle_frame` → `dispatch_request` / `dispatch_notification`. The notification-never-replies rule is structural — notifications route through a path that returns a liveness `bool` and only ever sends `event` pushes, never a reply frame.)*

- [x] **[hub outcome] Peel `Unchanged` / `TurnRunning` off `handle_set_model`; map residual `Ignored` → `SESSION_NOT_LIVE` in the dispatcher (Decision 8).**
      ...
      Verification: hub unit tests for same-model → `Unchanged`, mid-turn → `TurnRunning`; a dispatcher test that a `set_model` from a non-attached conn (guard `Ignored`) resolves as `SESSION_NOT_LIVE`, not success. *(Landed with hub tests `set_model_to_current_selection_is_unchanged`, `set_model_while_turn_running_is_rejected`, `set_model_on_unattached_connection_is_ignored`.)*

- [x] **[remaining requests] Convert `list_workspaces`, `list_providers`, `set_model`, `add_allow_pattern`, `delete_session`.**
      ...
      Verification: client actions `await` and update state at the call site; invalid `set_model` rejects instead of hanging; a delete refused as `NOT_OWNER` leaves the session in the list; a session mid-delete can't be re-opened or tasked; a delete whose response is dropped by a forced disconnect does not resurrect on reconnect (settled by the idempotent re-delete, not by catalog absence). *(Landed: `deleting` flag on `OpenedSession`; `requestDelete` + `resendPendingDeletes`; dropped-connection is distinguished from a real error via `json-rpc-2.0`'s `DefaultErrorCode` (`0`) — see `isServerError` — since `rejectAllPendingRequests` and a failed send both reject with code `0`, distinct from every app `RpcCode`. The `delete_session` result carries the fresh catalog and is merged; `list_workspaces` replaces.)*

- [x] **[notifications] Convert `task`, `resume`, `abort`, `close_session` to the adapter's `notify`; `event`/`session_evicted` to server notifications; replace the client `onmessage` type-switch with `addMethod` push handlers.**
      *Every* task appends + sets `running` only when `notify("task")` returns `true`; a draft/new session additionally `await`s its `open_session` result first (Decision 10). `submitApprovals` becomes `Promise.allSettled` over the best-effort `add_allow_pattern` requests (rejections only log activity), then `notify("resume")`, clearing the approval/allow drafts only when that returns `true` (Decision 11).
      ...
      Verification: a full turn streams (llm chunks, tool start/end, suspend→resume, abort) identically to `main`; a new session whose open is forced to fail shows the error and never a hung spinner; a rejected allow-pattern still resumes the approved call; `pnpm --filter coda-web lint` clean. *(Landed: the re-attach-busy edge — hit when the hub closes our session and another client grabs it before we re-attach — pushes a `session_evicted` notification (client → `applyHeldElsewhere("evicted")`) rather than inventing a `session_busy` push, keeping the S→C notification set to `event`/`snapshot`/`session_evicted`.)*

- [x] **[cleanup] Drop connect-time eager catalog pushes; decide the post-open/delete catalog path per the Risks section; delete dead `ClientMessage`/`ServerMessage` enums.**
      Purpose: removes the redundancy the old shape carried.
      Verification: first paint populates sidebar + model selector from the client's own requests; `cargo clippy` + `cargo test` + oxlint all clean. *(Landed: connect-time pushes dropped — the client's own `list_workspaces`/`list_providers` requests are the sole source; the post-open catalog push is dropped (client `applySnapshot` inserts the session via `upsertCatalogSession`); `delete_session`'s result carries the catalog. Verified: first paint populated sidebar + model selector against a live server.)*
