...
- [x] [config] Add `auto_compact_threshold: Option<u32>` to `ModelConfig`/TOML parsing (validated against `context_window`, mirroring `parse_max_completion_tokens`); resolve it on `ProviderHandle` (configured value, or 80% of `context_window`); thread it to `ModelProfile.auto_compact_threshold_tokens` in `open_session`.
      Purpose: gets the threshold from TOML to where the driver can read it, keeping the 80%-default policy in `coda_server`.
      Verification: config-parsing tests for unset, explicit value, and the exceeds-`context_window` validation error, all green. Every other `ModelProfile` construction site (test fixtures in `coda_agent` and `coda_server`) sets the new field to `u32::MAX` — effectively disabled — since none of those tests exercise auto-compaction; the driver-level tests in the next step build their own low threshold.
- [x] [core logic] In `driver.rs`, add `maybe_auto_compact`, called before every `ResumePoint::Generation` step when `is_root_thread` is true: compare last recorded `CompletionUsage.total_tokens` to the threshold, call `compaction::cutoff` with the current turn protected, and on `Some`, run the summarization request raced against `self.cancel`, appending the result via `self.agent.add_message`.
      Purpose: the actual auto-trigger; built last because it depends on every earlier step.
      Verification: three driver-level tests in `driver_tests/auto_compact.rs`, all green: (1) a three-round "second" turn crosses threshold after its first tool call — the summary's `cutoff` points at the previous turn's last message, its `turn_id` is the current turn, `message_view::model_view` shows the summary leading every one of the current turn's own messages in original order, and a further over-threshold check later in the same turn does not compact again; (2) a stateful sub-agent whose own second invocation goes over threshold (and would be a legal compaction target by turn-tagging alone) never gets a compaction record — proving the root-thread guard; (3) a scripted summarization failure followed by a later over-threshold check in the same turn does retry and succeed.
- [x] [verification] `cargo clippy` and `cargo test` across the workspace: clean.
      Purpose: project-standard correctness gate.
      Verification: `cargo test --workspace` and `cargo clippy --workspace --all-targets` both clean throughout. **Not done**: a manual run of a long multi-tool-call session against a real provider to eyeball post-compaction coherence — this sandbox has no configured LLM provider to run one against. Left for the user to do before relying on this in production; the reordering behavior itself is covered by the driver-level test above, just not against a real model's actual behavior.

## Deviations from Design

- **Fixed (load-bearing, caught in review): the manual path recorded the wrong `cutoff`.** `compact()` computes `cutoff_id` from `checkpoint.messages` before `command_message` (the `/compact ...` line) exists, then commits `[command, outcome]` together — so `command` physically lands *between* `cutoff_id`'s target and the summary. Recording `cutoff_id` itself as the summary's `cutoff` left `command` inside `model_view`'s tail, leaking the raw `/compact ...` line to the model on every manual compaction — silently, since no existing test built the commit in this exact shape (the unit tests exercised `cutoff`/`summary_message` in isolation; the hub tests never inspected `model_view` post-compaction). Fixed by recording `command.message_id()` as the cutoff instead, since `command` is what actually precedes the summary once committed. A new test, `manual_compaction_hides_its_own_command_line_from_the_model_view` (`compaction_tests.rs`), reproduces the exact commit shape and pins the fix. This doesn't change the `cutoff` field's meaning or any documented interface — it was a call-site bug in `compact()`, not a design error.
...
- **Fixed (load-bearing, caught in review): the auto-compaction record never reached the live snapshot.** `maybe_auto_compact` appended its summary/failure message straight to `Agent`'s in-memory history with no `AgentEvent`, so `SessionHub::fold_settled_turn` — which rebuilds `live.snapshot` purely by replaying the turn's `WireEvent` log — had no way to include it. A session that stayed live (no reattach forcing a reload from the checkpoint) would silently never show "Context compacted" or a failure record, even though storage had it from the moment the turn settled. Fixed by adding `AgentEvent::Custom`/`WireEvent::Custom`, emitted right after the message is appended and folded by `fold_settled_turn` the same way `LLMEnd`/`ToolCallEnd` already are (root-thread only, matching what the root's checkpoint holds). The frontend gained the matching `WireEvent` case, rendering it live through the same `historyToEntries` path a reload already used. Regression test: `hub::tests::replay::auto_compaction_reaches_the_live_snapshot_without_a_reattach`.
