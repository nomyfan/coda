...
>   `FsNoticeStore` 的 sidecar 文件与 spool 的 archive root 都失去了归宿，`delete_session`
>   的目录级联清理也不复存在；
> - `UserMessage.origin` 已被 main 用于 sub-agent thread 的来源标记，与本文的
>   `UserOrigin::TaskNotice` 同名冲突；
> - `BuildContext` 已换基座（`todo_store` 移除、新增 `file_locks`）；
> - 本文成文时尚不存在的相邻语义：per-session 权限模式、fork、rewind、turn 中途自动
>   compaction、PTC。
