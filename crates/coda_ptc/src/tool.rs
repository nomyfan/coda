...
pub fn run_javascript_definition(available: &[ToolDefinition]) -> ToolDefinition {
    ...
    ToolDefinition {
        ...
        description: format!(
            "Run one bounded ES2020 JavaScript program to coordinate several tool calls without returning intermediate results to the model. Top-level await and return are supported. Await every tool Promise before returning. Use Promise.all for independent calls expected to succeed, or Promise.allSettled when one call may fail, so no call remains unfinished. Each API takes exactly one object and resolves to the tool's raw string result; call JSON.parse only when that result is JSON, and never eval tool output. console.log(...) is the only console method and is available as bounded diagnostic output. Return the final compact JSON-serializable value. There are no ambient filesystem, network, process, timer, module, or require APIs; external effects are available only through the APIs below. A capability may reject with TOOL_UNAVAILABLE if policy tightens while the script runs. Returning with an unfinished tool call produces UNAWAITED_TOOL_CALLS and cancels that call.\n\nAvailable APIs:\n{}",
            api_lines.join("\n")
        ),
