            ...
            ResumePoint::PendingApproval {
                ...
                match &envelope.body {
                    ...
                    EnvelopeBody::Resume(decision) => {
                        ...
                        // A decision answers one batch of calls, and an
                        // unnamed call counts as rejected below — so a decision
                        // meant for an *earlier* batch would reject this one
                        // outright. That is not hypothetical: a second submit
                        // of the same approval (a double click, a retry after a
                        // reconnect) arrives once this thread has run those
                        // calls and suspended on the model's next batch.
                        // Naming none of what is pending is what identifies
                        // such a resume; stay suspended and re-announce instead,
                        // so the caller resynchronizes on what is really parked
                        // here.
