...
async fn an_approval_resumes_a_session_that_never_wrote_a_runtime_snapshot() {
    ...
    // killed mid-approval — and a session a fork has just minted — comes back
    // with checkpoints but no snapshot at all. A decision names the thread it
    // belongs to, so it must still reach it: routing it through the snapshot's
