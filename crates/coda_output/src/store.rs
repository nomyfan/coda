    ...
    fn reserve(
        ...
        for attempt in 0..65 {
            let failure = {
                ...
                let failure = if new
                    && (ledger.len() >= 65536
                        || ledger.values().filter(|e| &e.owner == owner).count() >= 4096)
                {
    ...
    fn materialize<'a>(
        ...
        Box::pin(async move {
            if self.failure.is_some() {
                return Err(
                    "OUTPUT_INCOMPLETE: tool executed but its complete output is unavailable"
                        .into(),
                );
            }
            let length = {
            ...
            let required = usize::try_from(length)
                ...
                .ok_or("OUTPUT_LIMIT: output size overflow")?;
            let lease = budget.reserve(required, cancel).await.map_err(|error| {
                format!("OUTPUT_LIMIT: tool executed; result delivery failed: {error:?}")
            })?;
                    ...
                    Source::Files(channels) => {
                        ...
                        for (channel, file, length) in channels {
                            ...
                            file.read_exact_at(&mut bytes, 0)
                                .map_err(|e| format!("OUTPUT_INCOMPLETE: {e}"))?;
                            append_text(&mut text, *channel, &String::from_utf8_lossy(&bytes));
                        }
                    }
                }
            ...
            tokio::select! {
                _ = cancel.cancelled() => Err("OUTPUT_ABORTED: result delivery cancelled after tool execution".into()),
                result = read => result.map_err(|e| format!("OUTPUT_INCOMPLETE: {e}"))?,
            }
        })
    }
}
...
impl Writer {
    ...
    fn finish(
        ...
        if self.temporary && self.directory.is_none() {
            self.spill()?;
        }
        if self.directory.is_none() {
            return Ok(Finished {
...
impl OutputStore for Store {
    ...
    fn begin(
        ...
        self.begin_capture(owner, channels, purpose, None)
    }
    ...
    fn retain_source(
        ...
        Box::pin(async move {
            ...
            {
                Ok(capture) => capture,
                Err(_) => return OutputData::unavailable(text, StorageFailure::Io),
            };
            capture.require_file();
            capture.set_deadline(deadline);
            for chunk in text.as_bytes().chunks(IO_BLOCK_BYTES) {
                if tokio::time::Instant::now() >= deadline {
                    capture.fail(StorageFailure::FinalizeTimeout);
                }
    ...
    fn retain(
        ...
        Box::pin(async move {
            ...
            {
                Ok(capture) => capture,
                Err(_) => return OutputData::Inline(text),
            };
            capture.require_file();
            capture.set_deadline(deadline);
            for chunk in text.as_bytes().chunks(IO_BLOCK_BYTES) {
...
impl OutputCapture for Capture {
    ...
    fn append(&mut self, channel: Channel, bytes: Vec<u8>) -> OutputFuture<'_, ()> {
        Box::pin(async move {
            assert!(bytes.len() <= IO_BLOCK_BYTES);
            self.reader
                .snapshot
                .lock()
                .unwrap()
            ...
            let operation = async {
                self.sender
                    .send(Request::Append(channel, bytes, reply))
                    .await
                    .map_err(|_| StorageFailure::Io)?;
    ...
    fn read(
        ...
        Box::pin(async move {
            ...
            if snapshot.sealed && snapshot.reference.is_none() {
                return Err("OUTPUT_INCOMPLETE: complete output was not retained".into());
            }
            let saved = snapshot
                .channels
                .iter()
            ...
            let pin = {
                ...
                let entry = ledger
                    .get_mut(&snapshot.id)
                    .filter(|entry| !entry.deleting)
                    .ok_or("OUTPUT_EXPIRED: retained file no longer exists")?;
                let pin = entry.pin.upgrade().unwrap_or_else(|| Arc::new(()));
                entry.pin = Arc::downgrade(&pin);
                pin
            };
...
impl Store {
    fn begin_capture(
        ...
        Box::pin(async move {
            ...
            {
                return Err("invalid output channels".into());
            }
            ...
            let memory = match &purpose {
                ...
                CapturePurpose::Programmatic(budget) => Some(
                    budget
                        ...
                        .await
                        .map_err(|e| format!("OUTPUT_LIMIT: {e:?}"))?,
                ),
            };
            ...
            let background = matches!(purpose, CapturePurpose::Background);
            let mut writer = Writer {
                ...
                snapshot,
                pin: Arc::new(()),
                temporary: matches!(purpose, CapturePurpose::Programmatic(_)),
                inline: channels.into_iter().map(|c| (c, Vec::new())).collect(),
                inline_limit: if background {
                    0
                } else {
                    (limit - 2 * IO_BLOCK_BYTES) / 8
                },
                directory: None,
                files: Vec::new(),
                failure: None,
